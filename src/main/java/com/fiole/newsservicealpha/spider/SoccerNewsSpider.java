...
public class SoccerNewsSpider implements PageProcessor {
    ...
    public void process(Page page) {
        ...
        if (pageUrl.contains("/GB/22149") || pageUrl.contains("/GB/22141")){
            peopleNewsExtract.addAllTargetLink(page,"hdNews clearfix");
            page.putField("pageType",1);
            page.putField("url",pageUrl);
        }
        else {
            peopleNewsExtract.pageExtract(page, ArticleTypeEnum.FOOTBALL);
        }
    }
    public Site getSite() {
        return site;
